...
export default function ProvinceMap({ province, width = 1120, height = 760 }: ProvinceMapProps) {
  ...
  return (
    <div
      ref={frameRef}
      className={`relative mx-auto aspect-[1120/760] w-[min(100%,1120px)] touch-none overflow-visible ${dragging ? "cursor-grabbing" : "cursor-grab"
        }`}
      onWheel={handleWheel}
...
function MemoryCard({
  ...
  return (
    <motion.article
      className={`absolute z-50 overflow-y-auto rounded-[8px] border border-[#D8DDD8] bg-[#FAFBF7]/94 text-[#5A6670] shadow-[0_18px_42px_rgba(90,102,112,0.18)] backdrop-blur ${expanded
    ...
    >
      ...
      {showMemory && (
        <>
          ...
          {showMemory && (
            <button
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-[7px] border border-[#D8DDD8] px-3 py-2.5 text-xs font-medium text-[#5A6670]/70 transition hover:border-[#A8C8DC] hover:text-[#A8C8DC]"
              type="button"
...
function LandmarkSprite({ city, lit }: Readonly<{ city: City; lit: boolean }>) {
  const className = `pixelated h-full w-full object-contain transition duration-500 ${lit
